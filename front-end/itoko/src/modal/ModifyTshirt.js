...
const ModifyTshirt = ({ graphicName, show, handleClose }) => {
  ...
  useEffect(() => {
    const tshirtProduct = paginationProduct.products.find(
      (product) => product.type === 'T-shirt' && product.name === graphicName
    );

    if (tshirtProduct) {
      setFormValues({
        description: tshirtProduct.description || '',
      ...
      setColors(Array.isArray(tshirtProduct.color) ? tshirtProduct.color : []);
      setSizes(Array.isArray(tshirtProduct.size) ? tshirtProduct.size : []);
      setCurrentImages(tshirtProduct.imageUrl || []);
    }
  ...
  const handleSaveChanges = async () => {
    try {
      ...
      if (!tshirtProduct) {
        throw new Error('Nessuna T-shirt trovata con il nome della grafica specificato.');
      }

      await axios.put(`${API_URL}/product/${tshirtProduct._id}`, formData, {
        headers: {
  ...
  return (
    <Modal show={show} onHide={handleClose} size="lg">
      <Modal.Header closeButton>
        <Modal.Title>Modifica T-shirt</Modal.Title>
      </Modal.Header>
